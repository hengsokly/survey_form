window.SurveyForm.SkipLogic = (function () {
  return {
    init: init,
    template: template,
    build: build
  ...
  function template() {
    var templateOject = {};
    templateOject[SurveyForm.FieldType.selectOne] = selectOne();
    templateOject[SurveyForm.FieldType.selectMultiple] = selectMultiple();
  ...
  function selectOne() {
    return {
      operators: [labelValue(locale.please_select, ''), labelValue('(=)', SurveyForm.SkipLogicConstant.EQUAL_OPERATOR), labelValue(locale.any_of, SurveyForm.SkipLogicConstant.MATCH_ANY_OPERATOR), labelValue("".concat(locale.skip_logic_not, " (!=)"), SurveyForm.SkipLogicConstant.NOT_OPERATOR)]
    };
  }

  function selectMultiple() {
    return {
      operators: [labelValue(locale.please_select, ''), labelValue('(=)', SurveyForm.SkipLogicConstant.EQUAL_OPERATOR), labelValue(locale.any_of, SurveyForm.SkipLogicConstant.MATCH_ANY_OPERATOR), labelValue(locale.match_all_of, SurveyForm.SkipLogicConstant.MATCH_ALL_OPERATOR), labelValue("".concat(locale.skip_logic_not, " (!=)"), SurveyForm.SkipLogicConstant.NOT_OPERATOR)]
    };
  }
  ...
  function getCodeList() {
    var data = [];
    var fields = $("input[value='".concat(SurveyForm.FieldType.selectOne, "'], input[value='").concat(SurveyForm.FieldType.selectMultiple, "']"));
  ...
  function buildValue(skipLogicForm, operator) {
    ...
    if (operator == SurveyForm.SkipLogicConstant.EQUAL_OPERATOR) {
      tagOption.maxTags = 1;
    }
